...
if __name__ == '__main__':
    ...
    datastream_2 = np.concatenate([
        rng.uniform(0, 0.5, size=(2000, 2)),
        rng.uniform(0.5, 1, size=(2000, 2)),
        rng.uniform(0.25, 0.75, size=(2000, 2))
    ])
    ...
    datastream_4 = np.concatenate([
        rng.uniform(0, 0.5, size=(2000, 1)),
        rng.uniform(0.5, 1, size=(2000, 1)),
        rng.uniform(0.25, 0.75, size=(2000, 1))
    ])

    datastream_5 = np.concatenate([
    ...
    for stream_index, stream in enumerate(streams):
        ...
        for index, x in enumerate(stream):
            ...
            if abcd.drift_detected:
                change_subspace_indices = [i + 1 for i in range(len(abcd.drift_dimensions)) if abcd.drift_dimensions[i] < abcd.tau]
                print(f"Change detected at index {index}")
                print(f"Change subspace is {abcd.drift_dimensions}")
