...
class EncoderDecoder(Protocol):
    def update(self, window, epochs: int):
        ...
        :param epochs: The number of training _epochs
        :return: nothing
        """

    def new_tuple(self, x: np.ndarray) -> Tuple[Any, Any, Any]:
        """
...
class AutoEncoder(nn.Module, EncoderDecoder):
    def __init__(self, input_size: int, eta: float):
        ...
        :param eta: The encoding factor. Hidden layer size is _eta * input_size
        """
    ...
    def update(self, window, epochs: int = 1):
        """
        Update the autoencoder on the given _window
