...
from river.drift.abcd_supplementary.feature_extraction import EncoderDecoderType
from river.ensemble.streaming_random_patches import BaseSRPEnsemble
...
class BaseSRPEnsembleABCD(BaseSRPEnsemble):
    ...
    def __init__(
        ...
        abcd_type: EncoderDecoderType = EncoderDecoderType.AE
    ):
        super().__init__(model, n_models, subspace_size, training_method, lam, drift_detector,
                         warning_detector, disable_detector, disable_weighted_vote, seed, metric)
        self.abcd = ABCD(seed=seed, model_type=abcd_type)
    ...
    def learn_one(self, x: dict, y: base.typing.Target, **kwargs):
        ...
        # ABCD
        self.abcd.update(x)
        if self.abcd.warning_detected:
            subspace_p_values = self.abcd.drift_dimensions
            change_subspace = [i for i in range(len(subspace_p_values)) if subspace_p_values[i] < self.abcd.tau]
            for model in self:
                intersection = np.intersect1d(change_subspace, model.features)
                if len(intersection) > 0:
                    model.trigger_warning(list(x.keys()), n_samples_seen=self._n_samples_seen)
        if self.abcd.drift_detected:
            subspace_p_values = self.abcd.drift_dimensions
            change_subspace = [i for i in range(len(subspace_p_values)) if subspace_p_values[i] < self.abcd.tau]
            for model in self:
                intersection = np.intersect1d(change_subspace, model.features)
                if len(intersection) > 0:
                    model.reset(list(x.keys()), n_samples_seen=self._n_samples_seen)
...
class BaseSRPEstimator:
    ...
    def trigger_warning(self, all_features, n_samples_seen: int):
...
class SRPABCDClassifier(BaseSRPEnsembleABCD, base.Classifier):
        ...
        if not self.disable_drift_detector and not self.is_background_learner:
            ...
            if not self.disable_background_learner:
                ...
                if self.warning_detector.drift_detected:
                    ...
                    self.trigger_warning(all_features=all_features, n_samples_seen=n_samples_seen)
...
class SRPABCDRegressor(BaseSRPEnsembleABCD, base.Regressor):
